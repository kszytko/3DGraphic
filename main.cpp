...
public:
	bool OnUserCreate() override
	{
		...
		matProj.m[0][3] = 0.5f * (float)ScreenWidth();
		matProj.m[1][3] = 0.5f * (float)ScreenHeight();
		matProj.m[2][3] = 0;
		matProj.m[3][3] = 1;

		matResize.m[0][0] = 0.5f;
		matResize.m[1][0] = 0;
		matResize.m[2][0] = 0;
		matResize.m[0][1] = 0;
		matResize.m[1][1] = 0.5f;
		matResize.m[2][1] = 0;
		matResize.m[0][2] = 0;
		matResize.m[1][2] = 0;
		matResize.m[2][2] = 1;
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		for (auto& tris : meshCube.tris) {
			...
			triRotatedX =  tris.multiplyByMatrix(matRotZ);
			//triRotatedXZ = triRotatedX.multiplyByMatrix(matRotZ);
			triRotatedXZ = triRotatedX;
			...
			triProjected = triResized.multiplyByMatrix(matProj);
		...
		for (auto& tris : triangles) {
			...
			DrawLine(tris.p[0].x, tris.p[0].y, 
				tris.cross.x , 
				tris.cross.y , 
				olc::YELLOW);
